...
class SchedulerConfig:
    ...
    # Qwen3-Next built-in MTP (upstream compatibility)
    # When enable_mtp=True, uses model's native MTP head via monkey-patch
    # instead of the external adapter system (spec_decode/mtp_adapters/).
...
class Scheduler:
    ...
    def _init_spec_decode(self) -> None:
        ...
        if self.config.speculative_method == "ngram":
            ...
            # Path A: Qwen3-Next built-in MTP (upstream compat)
            if self.config.enable_mtp:
                if hasattr(self.model, "mtp") and self.model.mtp is not None:
                    logger.info(
                        "[MTP] Using built-in Qwen3-Next MTP "
                        "(num_draft_tokens=%d, optimistic=%s)",
                        self.config.mtp_num_draft_tokens,
                        self.config.mtp_optimistic,
                    )
                    # _install_mtp() will be called in _create_batch_generator()
                    return
                else:
                    logger.warning(
                        "[MTP] enable_mtp=True but model has no built-in MTP head. "
                        "Falling back to external MTP adapter.",
                    )

            # Path B: External MTP adapters (DeepSeek V3, GLM-5, etc.)
    ...
    def _create_batch_generator(
        ...
        # Install Qwen3-Next built-in MTP if configured
