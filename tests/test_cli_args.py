...
def _make_parser(*, positional_model: bool = False, distributed: bool = False):
...
def _parse(argv, *, positional_model: bool = False, distributed: bool = False):
    """Parse *argv* through a standard parser and return the namespace."""
    parser = _make_parser(positional_model=positional_model, distributed=distributed)
...
# ---------------------------------------------------------------------------
# validate_serve_args — distributed
# ---------------------------------------------------------------------------


def test_validate_distributed_without_batching():
    """--distributed without --continuous-batching should exit."""
    args = _parse(
        ["--model", "m", "--distributed", "--dist-num-ranks", "2"],
        distributed=True,
    )
    with pytest.raises(SystemExit):
        validate_serve_args(args)
...
def test_validate_distributed_with_mllm():
    """--distributed with --mllm should exit."""
    args = _parse(
        [
            "--model", "m",
            "--distributed",
...
def test_validate_distributed_without_ranks():
    """--distributed without any rank/host specifier should exit."""
    args = _parse(
        ["--model", "m", "--distributed", "--continuous-batching"],
        distributed=True,
    )
    with pytest.raises(SystemExit):
        validate_serve_args(args)
...
def test_rebuild_excludes_distributed_args():
    """Distributed-only flags must NOT appear in rebuilt args."""
    args = _parse(
        [
            "--model", "m",
            "--continuous-batching",
    ...
    assert "--distributed" not in rebuilt
    assert "--dist-num-ranks" not in rebuilt
    assert "--dist-backend" not in rebuilt
